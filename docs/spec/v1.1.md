...
The Overlay Wizard populates its stream profile selector by polling the `frame-ingest-video` stats API — the same endpoint used by the telemetry overlay. No shared file or cross-container DB read is required.
...
Implementation note (2026-06-07): the unified installer currently deploys the implemented Portal
and Discord Audio Bridge services in LAN mode. It rejects HYBRID mode and capabilities whose
services are not implemented yet, preventing partial deployments from being represented as
complete. Shared Traefik/tunnel support remains required before HYBRID mode is enabled.
...
- **Stream profile sync method** — Overlay Wizard polls `frame-ingest-video` stats API on page load and manual refresh. See §2.0-D.
...
- Overlay Wizard UX flow and preset format (`overlay-presets.json` vs sqlite)
