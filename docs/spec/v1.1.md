...
Portal may run without authentication on a trusted LAN. Basic authentication is required in Hybrid
mode. The Portal must display when fallback configuration is active and distinguish Ready, Needs
setup, Offline, and Disabled tools.

Portal Basic authentication credentials must only be transported over HTTPS.
...
SFPG (Single File PHP Gallery) is bundled as part of the `frame-gallery` container and serves the per-day gallery from `/data/galleries/YYYY-MM-DD/`.

The `/today/gallery` wrapper page embeds the SFPG-generated gallery page in an `<iframe>` and polls `DATA_ROOT/state/latest.json` every second. When `updated_at` changes, the wrapper forces an iframe reload — no custom gallery logic required.

#### Thumbnail Strategy (V1)

SFPG generates thumbnails automatically as part of its gallery rendering, stored alongside gallery images (typically in a `.thumbs/` subfolder within the day folder).

The Viewer Remote (`/today/remote`) sources its optional thumbnail from the SFPG-generated thumb for `<base>.jpg`. This is **best-effort**: if the thumb does not yet exist, the remote shows nothing. No separate thumbnail generation step is added to the pipeline.
...
Updated by the Photo Pipeline after each successful publish (atomic temp + rename).

#### Fields

| Field | Type | Required | Format / Notes |
|---|---|---|---|
| `updated_at` | string | yes | ISO 8601 UTC — e.g. `"2025-12-29T03:14:07Z"` |
...
| `count_today` | integer | no | Total images successfully published today. 0 or absent if none. |
...
- `updated_at` uses UTC so all consumers get an unambiguous timestamp. The `.env TIMEZONE` setting is used for wall-clock display only (naming, day-folder selection), not for this field.
...
- `count_today` lets UI components (viewer, remote, portal status) know the list size without scanning the directory.
...
- `latest.json` is updated only after successful publish (`.ready` present). Rejections do not update `latest.json`.
...
- **Default gallery implementation** — SFPG bundled in `frame-gallery`; `/today/gallery` iframe wrapper refreshes on `latest.json` change. See §2.0-G.
- **Thumbnail strategy** — Viewer Remote sources thumbnails from SFPG-generated thumbs; best-effort, no pipeline step. See §2.0-G.
...
- Public access works for `/today/*`, `/overlays/view/*`, `/gallery/*`, `/audio/listen/*`, `/audio/hls/*`.
