...
- Discord Audio Bridge status + operator links
- System Status
...
The restart action and status/log collection require `frame-portal` to have access to the Docker socket (`/var/run/docker.sock`), mounted read-write. This is a standard pattern for management UIs but grants significant host access — operators should be aware this is a deliberate security trade-off in exchange for operational convenience.
