...
.\stack.cmd install --enable frame-belabox-manager
...
Belabox Manager adds a LAN-only `/belabox` operator page, an authenticated `/mqtt` WebSocket path for outbound Belabox agents, `/belabox-chunks` for tokenized chunk uploads, and `/belabox/remote` for the FRAME-authenticated remote belaUI proxy. `BELABOX_MQTT_PASSWORD` is generated by the installer; SSH settings are only for future install, diagnostics, update, or removal flows.
