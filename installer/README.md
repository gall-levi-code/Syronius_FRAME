...
Belabox Manager adds a LAN-only `/belabox` page and an authenticated `/mqtt` WebSocket path for outbound Belabox agents. `BELABOX_MQTT_PASSWORD` is generated by the installer; SSH settings are only for future install, diagnostics, update, or removal flows.
