...
.\stack.cmd install --mode HYBRID --public-hostname frame.example.com --enable frame-belabox-manager
...
Belabox Manager adds a LAN-only `/belabox` operator page, an authenticated `/belabox/control`
WebSocket path for outbound Belabox agents, `/belabox-chunks` for tokenized chunk uploads, and the
FRAME-authenticated `/belabox/remote` and `/belabox/mixer` tools. The mixer proxy maps only to
`127.0.0.1:9080` on the selected Belabox; it never publishes that port or accepts a browser-supplied
target. The manager provisions a unique control secret to each device during SSH installation;
there is no shared broker credential. SSH settings are otherwise only for install, diagnostics,
update, or removal flows.

Belabox Manager cannot be enabled in LAN mode. Agent install and repair require a configured public
`wss://.../belabox/control` endpoint so the installer cannot report success for an agent that has no
remote path back to FRAME. The installer derives that endpoint from `EDGE_PUBLIC_BASE_URL`; it is
not an advanced override, so changing the public hostname cannot leave agents pointed at a stale
endpoint.
...
When Belabox Manager is enabled, that allowlist includes `/belabox/remote`, `/belabox/mixer`,
`/belabox/control`, and `/belabox-chunks`, but continues to exclude `/belabox`, `/belabox/api`, and
`/belabox/assets`. Browser tools use FRAME login; the device-only control route uses its own
nonce/HMAC authentication.
...
- Belabox Manager installation is refused outside Hybrid mode or without a public
  `wss://.../belabox/control` endpoint.
