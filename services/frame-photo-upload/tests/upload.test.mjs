...
test("streams one completed upload into staging", async () => {
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}/photos/api/upload`, {
    method: "POST",
    headers: { authorization: `Basic ${Buffer.from("frame:secret").toString("base64")}`, "x-frame-transfer-id":"transfer-test-1", "x-frame-file-size":"22" },
    body: data,
  });
  ...
  const progress = await fetch(`http://127.0.0.1:${address.port}/api/internal/photo-upload/progress`, { headers:{authorization:"Bearer test-service-token"} });
...
test("exposes live receiving progress while a web upload is still streaming", async (t) => {
  ...
  const app = await createApp({ dataRoot:root,maxInputBytes:4096,publicDir:path.resolve("public"),auth:{username:"frame",password:"secret",realm:"FRAME Test"},serviceToken:"test-service-token" });
  const server=app.listen(0);await once(server,"listening");t.after(()=>server.close());
  const port=server.address().port;const boundary="frame-test-boundary";
  const preamble=`--${boundary}\r\nContent-Disposition: form-data; name="photo"; filename="slow.jpg"\r\nContent-Type: image/jpeg\r\n\r\n`;
  const ending=`\r\n--${boundary}--\r\n`;
  const responsePromise=new Promise((resolve,reject)=>{
    const request=http.request({host:"127.0.0.1",port,path:"/photos/api/upload",method:"POST",headers:{authorization:`Basic ${Buffer.from("frame:secret").toString("base64")}`,"content-type":`multipart/form-data; boundary=${boundary}`,"x-frame-transfer-id":"slow-transfer","x-frame-file-size":"10"}},resolve);
    request.on("error",reject);request.write(preamble);request.write("12345");
    t.after(()=>request.destroy());
    setTimeout(()=>{request.write("67890");request.end(ending);},80);
  });
  await new Promise((resolve)=>setTimeout(resolve,30));
  const progress=await fetch(`http://127.0.0.1:${port}/api/internal/photo-upload/progress`,{headers:{authorization:"Bearer test-service-token"}}).then((response)=>response.json());
  ...
  const response=await responsePromise;assert.equal(response.statusCode,202);response.resume();
});
...
test("cleans up an interrupted upload without destabilizing the service", async (t) => {
  ...
  const app = await createApp({ dataRoot:root,maxInputBytes:4096,publicDir:path.resolve("public"),auth:{username:"frame",password:"secret",realm:"FRAME Test"},serviceToken:"test-service-token" });
  const server=app.listen(0);await once(server,"listening");t.after(()=>server.close());
  const port=server.address().port;const boundary="frame-abort-boundary";
  const request=http.request({host:"127.0.0.1",port,path:"/photos/api/upload",method:"POST",headers:{authorization:`Basic ${Buffer.from("frame:secret").toString("base64")}`,"content-type":`multipart/form-data; boundary=${boundary}`,"x-frame-transfer-id":"aborted-transfer","x-frame-file-size":"100"}});
